...
def main() -> None:
    ...
    if distributed and not use_cuda:
        raise RuntimeError("Distributed execution requires CUDA in this script")
    ...
    compiled_model = torch.compile(base_model, dynamic=False, fullgraph=True) if compile_requested else base_model
    model: nn.Module = (
    ...
    def autocast_ctx():
        return torch.autocast(device_type=device.type, dtype=torch.bfloat16) if use_cuda else nullcontext()
