...
class Hyperparameters:
    ...
    # Model shape. num_layers=11 enabled by int6 savings (~2MB less than int8).
    ...
    # Sliding window eval at stride=64 gives ~0.032 BPB vs non-overlapping.
    eval_stride = int(os.environ.get("EVAL_STRIDE", 64))
    eval_batch_seqs = int(os.environ.get("EVAL_BATCH_SEQS", 32))

    # BigramHash: hash consecutive token pairs into a learned embedding table.
    bigram_vocab_size = int(os.environ.get("BIGRAM_VOCAB_SIZE", 2048))
    bigram_dim = int(os.environ.get("BIGRAM_DIM", 128))

    # SWA: average checkpoints from last portion of warmdown.
...
# -----------------------------
# POST-TRAINING QUANTIZATION (INT6 mixed with INT8 fallback)
# -----------------------------
# Attention + MLP matrices → INT6 (64 levels, saves ~2MB vs INT8, enables 10-11 layers).
# Embedding → fp16 passthrough (~0.006 BPB improvement vs INT8 embedding).
# Control tensors (scales, mixes) → fp32 passthrough.
...
class Rotary(nn.Module):
    """RoPE with NTK-aware dynamic scaling: auto-adjusts base freq when seq_len > train_seq_len."""

    def __init__(self, dim: int, base: float = 10000.0, train_seq_len: int = 2048):
        super().__init__()
        self.dim = dim
        self.base = base
    ...
    def forward(
        ...
        if (
            ...
            if seq_len > self.train_seq_len:
                ...
                adj_base = self.base * (scale ** (self.dim / (self.dim - 2)))
                inv_freq = 1.0 / (
                    adj_base
                    ** (
...
class SmearGate(nn.Module):
    """Blend each token's embedding with the previous token's (learned gate)."""
...
class GPT(nn.Module):
    def __init__(
        ...
        bv = bigram_vocab_size
        self.bigram = BigramHashEmbedding(bv, bigram_dim, model_dim) if bv > 0 else None
        ...
        for name, module in self.named_modules():
            if isinstance(module, nn.Linear):
                if getattr(module, "_zero_init", False):
                    ...
                    # OrthoInit: orthogonal init for better gradient flow; scale down projections
                    nn.init.orthogonal_(module.weight, gain=1.0)
                    if ".proj." in name or name.endswith(".proj"):
                        with torch.no_grad():
                            module.weight.mul_(1.0 / math.sqrt(2 * num_layers))

    def _run_encoder_decoder(self, x: Tensor) -> Tensor:
        x0 = x
        skips: list[Tensor] = []
...
# -----------------------------
# Overlapping windows at stride=64 give each position up to seq_len-64 prior context.
# Worth ~0.032 BPB vs non-overlapping 1024-token chunks.
...
def eval_val_sliding(
    ...
    my_windows = window_starts[
        (total_windows * rank) // world_size : (total_windows * (rank + 1))
        // world_size
    ]
        ...
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            if rank == 0 and (bi // batch_seqs) % 50 == 0:
                ...
                if token_count.item() > 0:
                    rbpb = (
                        (loss_sum / token_count).item()
                        / math.log(2.0)
                        * (token_count.item() / byte_count.item())
                    )
                print(
                    f"  sliding_eval [{done / len(my_windows) * 100:5.1f}%] {done}/{len(my_windows)} windows running_bpb={rbpb:.6f}",
                    flush=True,
                )
...
def main() -> None:
    ...
    _smi = subprocess.run(["nvidia-smi"], capture_output=True, text=True, check=False)
    ...
    # Optimizer split: tok_emb (AdamW), matrix params (Muon+wd), scalars (AdamW)
    ...
    log0(
        f"num_layers:{args.num_layers} mlp_mult:{args.mlp_mult} model_dim:{args.model_dim}"
    )
    log0(
        f"attention_mode:gqa num_heads:{args.num_heads} num_kv_heads:{args.num_kv_heads}"
    )
    log0(
        f"tie_embeddings:{args.tie_embeddings} embed_lr:{token_lr} matrix_lr:{args.matrix_lr} muon_wd:{args.muon_weight_decay} scalar_lr:{args.scalar_lr}"
    )
    log0(
    ...
    log0(
        f"bigram_vocab_size:{args.bigram_vocab_size} bigram_dim:{args.bigram_dim} eval_stride:{args.eval_stride} swa_enabled:{args.swa_enabled} seed:{args.seed}"
    )
    ...
    # Warmup: prime compiled paths, then restore init state so training starts clean.
    ...
    # SWA: replace weights with averaged checkpoints
    if args.swa_enabled and swa_state is not None and swa_count > 1:
        log0(f"swa:applying averaged {swa_count} checkpoints")
        cur = base_model.state_dict()
        base_model.load_state_dict(
            {
                name: (t / swa_count).to(dtype=cur[name].dtype)
                for name, t in swa_state.items()
            },
            strict=True,
        )

    # Serialization: export INT6+zstd/zlib artifact.
    if master_process:
    ...
    quant_blob = (
        ...
        if _COMPRESSOR == "zstd"
        else zlib.compress(quant_raw, level=9)
    )
    if master_process:
        with open("final_model.int8.ptz", "wb") as f:
            f.write(quant_blob)
        log0(
            f"Serialized model int6+{_COMPRESSOR}: {os.path.getsize('final_model.int8.ptz')} bytes  total: {os.path.getsize('final_model.int8.ptz') + len(code.encode('utf-8'))} bytes"
        )
    ...
    quant_state = torch.load(
        io.BytesIO(decompressed), map_location="cpu", weights_only=False
    )
    base_model.load_state_dict(
        dequantize_mixed_int6(quant_state["w"], quant_state["m"], sd_cpu), strict=True
    )

    # Final eval: sliding window if configured, else standard non-overlapping
    torch.cuda.synchronize()
